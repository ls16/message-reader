...
### Сборка сервера для чтения сообщений
  Сборка сервера производится функцией *build(options: Object): Server*, которая возвращает экземпляр сервера:
  ...
  Пример:
    ...
    const server = MR.build({regexp, grammar});

### Запуск сервера на прослушивание входящий соединений
  Запуск на прослушивание производится методом сервера *listen(options: Object): Server*. Внутри сервера для транспорта данных испольуются объекты стандартных классов *node*: [net.Server][net-server] или [tls.Server][tls-server].
  Опции:
...
### Постобработка распознанных сообщений и ошибок
  ...
  Для обработки ошибок, возникших в процессе распознавания сообщения, можно определить обработчик, подписавшись на событие *error* объекта *server*.
  ...
  У объектов *ctx* и *evt* определено специальное свойство *socket: [net.Socket][net-socket]* - которое можно использовать для передачи данных клиенту. Дополнительно у *evt* определено свойство *error*, содержащее информацию о произошедшей ошибке.
